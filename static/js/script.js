...
function showPage(pageName) {
  ...
  pages.forEach((id) => {
    document
      .getElementById(id)
      ?.classList.toggle("hidden", id !== `page-${pageName}`);
  });
  navs.forEach((id) => {
    document
      .getElementById(id)
      ?.classList.toggle("active", id === `nav-${pageName}`);
  });
...
function addLog(type, message, timestamp, data) {
  ...
  switch (type) {
    ...
    case "sort":
      ...
      prefix = `[SORT]`;
      message = `Phân loại ${data.name}, tổng: ${data.count}`;
      break;
    case "pass":
      colorClass = "text-indigo-400";
      prefix = `[PASS]`;
      message = `Đếm vật phẩm đi thẳng qua ${data.name}, tổng: ${data.count}`;
      break;
    case "qr":
      colorClass = "text-blue-400";
      prefix = `[QR]`;
      const qr_lane_index = Object.keys(laneIdMap).find(
        (key) => laneIdMap[key] === data.data_key
      );
    ...
    case "qr_ng":
      ...
      prefix = `[QR]`;
      message = `Hàng NG: ${data}`;
      showQrOverlay(data, "bg-red-600");
      break;
    case "unknown_qr":
      colorClass = "text-yellow-500";
      prefix = `[QR]`;
      message = `Không rõ: ${data.data_key}`;
      showQrOverlay(data.data_key, "bg-yellow-500");
      break;
  }
...
function updateState(state) {
  ...
  state.lanes.forEach((lane, i) => {
    ...
    if (laneSensorEl) {
      ...
      laneSensorEl.classList.toggle(
        "sensor-inactive",
        lane.sensor_reading !== 0
      );
    }

    const grabEl = document.getElementById(`lane-${i}-grab`);
    if (grabEl) {
      grabEl.classList.toggle(
        "relay-active",
        isSortingLane && lane.relay_grab === 1
      );
      grabEl.classList.toggle(
        "relay-inactive",
        isSortingLane && lane.relay_grab !== 1
    ...
    if (pushEl) {
      pushEl.classList.toggle(
        "relay-active",
        isSortingLane && lane.relay_push === 1
      );
      pushEl.classList.toggle(
        "relay-inactive",
        isSortingLane && lane.relay_push !== 1
    ...
    if (statusEl) {
      ...
      const isWaiting =
        lane.status.includes("Đang chờ") ||
        lane.status.includes("Đang phân loại") ||
        lane.status.includes("Đang đi thẳng");
    ...
    if (mockStatusEl) {
      ...
      mockStatusEl.textContent = isActive
        ? "ĐANG KÍCH HOẠT (LOW)"
        : "KHÔNG KÍCH HOẠT (HIGH)";
  ...
  document
    .getElementById("mock-badge")
    .classList.toggle("hidden", !state.is_mock);
  document
    .getElementById("maintenance-badge")
    .classList.toggle("hidden", !state.maintenance_mode);

  document
    .getElementById("mock-controls")
    .classList.toggle("hidden", !state.is_mock);
    ...
    document
      ...
      .forEach((el) => {
        const allowDuringMaintenance =
          el.dataset && el.dataset.allowMaintenance === "true";
...
function renderAllLanesUI(lanes) {
  ...
  const relayTestContainer = document.getElementById(
    "manual-relay-test-container"
  );
    ...
    const laneStatusHTML = `
            <div id="lane-${i}" class="border border-gray-700 p-3 rounded-md">
                <div class="flex justify-between items-center mb-1">
                    <span id="lane-${i}-name" class="font-bold">${
      lane.name
    }</span>
                ...
                <div class="flex justify-between text-xs ${
                  hasSensor ? "" : "opacity-30"
                }">
                    <span>Cảm biến:</span>
                    <div id="lane-${i}-sensor" class="sensor-light ${
      hasSensor ? "sensor-inactive" : ""
    }"></div>
                </div>
                <div class="flex justify-between text-xs mt-1">
                    <span>Relay Thu:</span>
                    <div id="lane-${i}-grab" class="relay-light ${
      isSortingLane ? "relay-inactive" : "relay-disabled"
    }"></div>
                </div>
                <div class="flex justify-between text-xs mt-1">
                    <span>Relay Đẩy:</span>
                    <div id="lane-${i}-push" class="relay-light ${
      isSortingLane ? "relay-inactive" : "relay-disabled"
    }"></div>
                </div>
            </div>
    ...
    const sensorTestHTML = `
            <div class="bg-gray-900 p-3 rounded-lg text-center ${
              hasSensor ? "" : "opacity-30"
                ...
                <div id="test-sensor-${i}" class="sensor-light ${
      hasSensor ? "sensor-inactive" : ""
    } mx-auto mt-1 w-6 h-6"></div>
            </div>
        `;
...
function updateQueueUI(qrQueueIndices) {
  ...
  if (qrQueueContainer && qrEmptyMessage) {
    qrQueueContainer
      .querySelectorAll(".queue-item-qr")
...
function loadConfig() {
  _fetch("/config")
    ...
    .then((cfg) => {
      document.getElementById("config-full-json").value = JSON.stringify(
        cfg,
        null,
        4
      );
  ...
  _fetch("/api/mock_gpio", {
    ...
    .then((data) => {
      if (data?.error) {
        ...
        addLog(
          "info",
          `Đã đặt cảm biến ${data.lane} về trạng thái ${stateLabel} (pin ${data.pin}).`
        );
      }
    })
    .catch((err) => {
...
function toggleAutoTest() {
  ...
  ws.send(
    JSON.stringify({ action: "toggle_auto_test", enabled: autoTestEnabled })
  );
...
function renderCharts(data) {
  ...
  const laneIndices = Object.keys(laneNamesMap)
    .map(Number)
    .sort((a, b) => a - b);
  ...
  const ctxDoughnut = document
    .getElementById("sort-chart-doughnut")
    ?.getContext("2d");
