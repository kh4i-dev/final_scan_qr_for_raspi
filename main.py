...
class MockGPIO(GPIOProvider):
    def __init__(self):
        self.BOARD = "mock_BOARD"; self.BOARD = "mock_BOARD"; self.OUT = "mock_OUT"
...
# maxBytes = 2_000_000 → Mỗi file log tối đa 2MB
# backupCount = 5 → Lưu tối đa 5 file cũ (system.log.1, system.log.2, ...)
...
AUTO_TEST_ENABLED = False # (Sửa Bug 2)
auto_test_last_state = [] # (Sửa Bug 2)
auto_test_last_trigger = [] # (Sửa Bug 2)
...
# Toàn bộ hàm này đã được thay thế bằng logic của appv2.py để sửa lỗi lặp mã QR
# =============================
            ...
            if data:
                ...
                if data != last_qr:
                    ...
                    if data_key in LANE_MAP:
                        ...
                        qr_source_text = f"(Quét bằng {qr_source}) " if qr_source else ""
                        broadcast_log({"log_type": "info", "message": f"{job_id_log_prefix} {qr_source_text}Vật vào (Camera Trigger). Ghép cặp: {job_status} -> Lane '{job_lane_name}'."})
                        
                        logging.info(f"[CAM_TRIG] {job_id_log_prefix} (CAM_ENTRY) Quét bằng {qr_source}. Ghép cặp: {job_status} -> Lane '{job_lane_name}'. Queue chính: {current_queue_len}")
                        
                        if stop_on_qr:
                            logging.info(f"[CAM_TRIG] Đã quét mã. DỪNG băng chuyền trong {stop_delay_qr}s.")
                            CONVEYOR_STOP()
                        ...
                        logging.warning(f"[CAM_TRIG] ({qr_source}) Không rõ mã QR: raw='{data_raw}', canon='{data_key}', keys={list(LANE_MAP.keys())}")
                    
                    # --- HẾT KHỐI KÍCH HOẠT ---

                else:
                    # data == last_qr
                    # Vẫn thấy mã QR cũ (do vật đang dừng hoặc di chuyển chậm).
                    # CHỈ cập nhật thời gian, KHÔNG làm gì cả.
                    # Điều này ngăn chặn lỗi lặp mã.
                    last_time = now
            
            else: 
                # data is None (không thấy mã QR)
                if last_qr is not None and (now - last_time > qr_debounce_time):
                    # Nếu đã qua thời gian debounce mà không thấy mã nào,
                    # reset 'last_qr'. Điều này cho phép hệ thống
                    # quét vật phẩm tiếp theo (kể cả khi nó có mã giống hệt vật cũ).
                    logging.info(f"[CAM_TRIG] Reset QR debounce (không thấy mã > {qr_debounce_time}s). Sẵn sàng quét mã tiếp theo.")
                    last_qr = None
            
            # ==================================
            # (CẬP NHẬT) KẾT THÚC LOGIC MỚI
            # ==================================
...
if __name__ == "__main__":
    try:
        ...
        logging.info("=========================================")
        logging.info("    HỆ THỐNG PHÂN LOẠI SẴN SÀNG (MERGED & FIXED)")
        if use_gantry_logic:
